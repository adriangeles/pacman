 ...
 * <p/>
 */
package es.upm.fi.f980092.tfc.gameengine.render;
...
public class ESTransform {
	...
	private float[] matrixMultiplyTmp = new float[16];
	...
	public ESTransform(float[] mMatrix) {
		mMatrixFloatBuffer = ByteBuffer.allocateDirect(16 * 4)
		.order(ByteOrder.nativeOrder()).asFloatBuffer();				
		this.mMatrix = mMatrix.clone();		
	}
	...
	public void perspective(float fovy, float aspect, float nearZ, float farZ) {
		double d = Math.tan(fovy / 360.0 * Math.PI) * nearZ;
		float frustumH = Math.round(d);
